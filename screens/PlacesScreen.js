...
const handleSubmit = () => {
  ...
  fetch(`https://api-adresse.data.gouv.fr/search/?q=${city}`)
    ...
    .then((data) => {
      ...
      fetch(`https://locapic-backend-cecileliao.vercel.app/places`, {
        ...
        .then((response) => {
          if (!response.ok) {
            throw new Error('Failed to register new place');
          }
          return response.json(); // Récupérer la réponse en tant qu'objet JSON
        })
        .then((data) => {
          if (data.result) {
        ...
        .catch((error) => {
          console.log(error);
          res.json({ error: 'Failed to register new place' });
        });
    })
    .catch((error) => {
      console.log(error);
      res.json({ error: 'Failed to fetch geographic data' });
    });
};
  ...
  const handleDelete = (placeName) => {
    fetch(`https://locapic-backend-cecileliao.vercel.app/places`, {
      ...
      .catch((error) => {
        console.log(error);
        res.json({ error: 'Failed to delete place' });
      });
  };
